...
>
> 데이터는 **온통청년 청년정책 공공 OpenAPI**로 연동. 첫 타깃: **인천광역시 연수구 · 청년(만 18~39세)**
...
| **진짜 해자** | 지자체 데이터 **수집·구조화·최신화** (기술이 아니라 운영력) |
...
수록(인천/연수구 청년): 청년월세 · 드림체크카드 · 면접수당 · 드림For청년통장 · 재직청년 복지포인트 · 마음건강.

---

## 🔌 공공 API 연동

- **온통청년 청년정책 OpenAPI** — 한국고용정보원, [data.go.kr/data/15143273](https://www.data.go.kr/data/15143273/openapi.do)
...
- **검증**: 정규화 로직 오프라인 셀프테스트 **10/10 통과**
  ```bash
  node youth-benefits-demo/scraper/fetch_youth_api.mjs --selftest
  ```

> ⚠️ **현실 체크**: 정부/지자체 사이트는 직접 크롤이 **HTTP 403**으로 막힌다(실측).
...
youth-benefits-demo/
  ...
  data/yeonsu_youth.json      검증 시드(공개정보 기반 폴백 데이터)
  scraper/fetch_youth_api.mjs 온통청년 공공API 연동 + 정규화 + 셀프테스트
  scraper/scrape.py           직접 크롤 골격(403 차단 실측용)
  scraper/run.sh              수집 실행 러너
  README.md                   데모 상세 문서
.github/workflows/
  youth-benefits.yml          공공API 수집 + Pages 자동 배포
```
